...
! Variables from directions input (defined in input file)
    ...
    Double Precision, dimension(:), allocatable :: w_s, m_s, std_s
    Logical :: scattering, outputIngoing, xyOutput, blurOn, GaussSpeeds, gauss_time
    Integer :: n_s
...
! Variables from timing inputs (defined in input file)
    ...
    Integer :: n_t
    Double Precision, dimension(:), allocatable :: w_t, m_t, std_t
...
! Variables for counter print out
    ...
    Integer, dimension(2) :: N_intersect            ! Counts trajectories that intersect at least one beam & hence are detected.
    logical :: any_beam
...
! Crude chopper parameters
   Double Precision :: chopPos, chopOpen, chopLen, chopClose, tChop
...
!******************************************
     ...
     Read(11,*) n_s                             ! No. of Gaussians for speed
      Allocate(w_s(n_s))                        ! allocate the weight, mean and standard deviation arrays before reading in the data
      Allocate(m_s(n_s))
      Allocate(std_s(n_s))
     Read(11,*) (w_s(i) , i = 1, n_s) 		 	    ! Weighting of speed Gaussians
     Read(11,*) (m_s(i) , i = 1, n_s)	    		! Mean of the speed Gaussians
     Read(11,*) (std_s(i) , i = 1, n_s)				! Standard deviation of the speed Gaussians
...
!****************************************
     ...
     Read(13,*) gauss_time                      ! TRUE/FALSE as whether to use Gaussians or Square wave for start time
     Read(13,*) pulseLength						! Discharge pulse length (If using square wave)
     Read(13,*) n_t                             ! No. of Gaussian time functions
        Allocate(w_t(n_t))                      ! allocate the weight, mean and standard deviation arrays before reading in the data
        Allocate(m_t(n_t))
        Allocate(std_t(n_t))
     Read(13,*) (w_t(i), i = 1 , n_t)               ! Weights of time gaussians
     Read(13,*) (m_t(i), i = 1 , n_t)               ! Means of the time Gaussians
     Read(13,*) (std_t(i), i = 1 , n_t)             ! Standard deviations of the time Gaussians
...
!*****************************************
! Read in parameters for crude chopper
!*****************************************

    Open(unit=17,file='Inputs/Chopper.inp')
    Read(17,*) chopPos
    Read(17,*) chopOpen
    Read(17,*) chopLen
    Close(17)

    chopOpen=chopOpen*1.0e-6
    chopLen=chopLen*1.0e-6
    chopClose=chopOpen+chopLen
...
!********************************************************************************
 ...
 !       write(*,*)'Calling ingoing speeds from Gauss'
		call ingoing_speed_from_Gauss(w_s, m_s, std_s, w_t, m_t, std_t, n_s, n_t, gauss_time, dist, pulseLength, particlespeed(1), particleTime(1))
...
!***********************************************
! CRUDE TIME FILTER (CHOPPER)
!***********************************************
                if(j .eq. 1) then                                                       ! For ingoing particles only
                        z = abs(particleStartPos(1,3) - chopPos)                        ! Calculate chopper position
                        tChop = abs(z / (particleSpeed(1)*particleVector(1,3))) + particleTime(1)       ! Time particle reaches chopper

!                       if(k .eq. 1) then
!                               write(*,*) tChop, particleTime(1), particleSpeed(1)*particleVector(1,3)
!                               write(*,*) z, chopPos
!                       endif

                        if ((tChop .lt. chopOpen) .or. (tchop .gt. chopClose)) then     ! If outside the time window
                                Intersect = .false.                                     ! set intersect to false (ingoing particle is not probed)
                                outOfBounds(j) = .true.
                                particleStartPos(j,1)=2.0*rmax                          ! }
                                particleStartPos(j,2)=2.0*rmax                          ! } Overwrite the x and y start position and vector to ensure that it missed
                                particleVector(j,1)=0.0                                 ! } the wheel so that the scattered particle is filtered out by the missWheel
                                particleVector(j,2)=0.0                                 ! } routine

                                particleStartPos(2,1)=2.0*rmax                          ! } Scattered particle trajectory is parallel to the axis
                                particleStartPos(2,2)=2.0*rmax                          ! } but 2*rmax off in both x and y
                                particleVector(2,1)=0.0                                 ! }
                                particleVector(2,2)=0.0                                 ! }
!				write(*,*) particleStartPos(2,1), particleStartPos(2,2)
                        endif
                endif
...
!***********************************************
	   ...
	   if(j .eq. 2) then							! We only look at scattered trajectories as 
		...
		if(hit .eqv. .false.) then					! If we have a trajectory missing the wheel we only add to the counter on the
			...
			if(k .eq. 1) then					! If we counted every rejection our stats would be a factor of npass too large
...
! Write Array Count stats
    ...
    Write(21,*) N_intersect(1), "Ingoing trajectories probed by at least one beam"
    Write(21,*) 1.0*N_intersect(1)/(ncyc-beyondMax(1)), " Fraction of trajectories within probe region actually probed "
    Write(21,*) acceptedIngoing, "Ingoing trajectories probed, including multiple intersections"
    Write(21,*) (ncyc-beyondMax(1))*npass, "Max theoretical ingoing probe events including multiple beams"
	Write(21,*) 1.0*acceptedIngoing/((ncyc-beyondMax(1))*npass), "Ingoing Probe efficiency (actual/max theory) (not very relevant for HC!)"
	
    if (Scattering) then        !No point in writing the stats out if they are not relevant, plus the missWheel will mess things up...
        Write(21,*) ncyc-missWheel, "Total Scattered trajectories"
        Write(21,*) N_intersect(2), "Scattered trajectories probed by at least one beam"
        Write(21,*) 1.0*N_intersect(2)/(ncyc-missWheel), " Fraction of trajectories actually probed"
        Write(21,*) (ncyc - missWheel)*npass, "Max theoretical scattered probe events including multiple beams "
        Write(21,*) acceptedScattered, "Actual scattered probe events including multiple intersections"
        Write(21,*) 1.0*acceptedScattered/((ncyc-missWheel)*npass), " Scattered probe efficiency (actual/max theory) (not very relevant for HC!) "
    end if
...
! Print info to screen
    ...
    print '(a, F4.2, a)', "        ", 1.0*missWheel/(ncyc-beyondMax(1)), " Fraction of trajectories missing the wheel (excluding those outside probe region) "
    print *, N_intersect(1), "Ingoing trajectories probed by at least one beam"
    print '(a, F4.2, a)', "        ", 1.0*N_intersect(1)/(ncyc-beyondMax(1)), " Fraction of trajectories within probe region actually probed "
    print *, acceptedIngoing, "Ingoing probe events, including multiple intersections "
    print *, (ncyc-beyondMax(1))*npass, "Max theoretical ingoing probe events including multiple beams"
    print '(a, F4.2, a)', "        ", 1.0*acceptedIngoing/((ncyc-beyondMax(1))*npass), " Ingoing probe efficiency (actual/max theory) (not very relevant for HC!)"
    if (Scattering) then        !No point in writing the stats out if they are not relevant, plus the missWheel will mess things up...
     print *, ncyc-missWheel, "Total Scattered trajectories"
     print *, N_intersect(2), "Scattered trajectories probed by at least one beam"
     print '(a, F4.2, a)', "        ", 1.0*N_intersect(2)/(ncyc-missWheel), " Fraction of trajectories actually probed"
     print *, (ncyc - missWheel)*npass, "Max theoretical scattered probe events including multiple beams "
     print *, acceptedScattered, "Actual scattered probe events including multiple intersections"
     print '(a, F4.2, a)', "        ", 1.0*acceptedScattered/((ncyc-missWheel)*npass), " Scattered probe efficiency (actual/max theory) (not very relevant for HC!) "
     print *,"        "
    end if


end program MCScatteringFM_v4
